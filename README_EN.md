...
- **Requires DeepSeek Harness on the 0.1.5 or 0.1.6 line** (declared via `dsh.plugin.json` `engines.dsh`; peer range `^0.1.5-alpha.1 || ^0.1.6-alpha.1`). Per node-semver rules each arm covers its line's pre-releases from alpha.1 plus the stable release when it lands; those floors deliberately stay at alpha.1 (alpha.x kernels are supported too, so narrowing them buys nothing). The build baseline is pinned to 0.1.6-alpha.1 (devDependencies) — compiling against the newest pre-release of the supported kernel lines.
