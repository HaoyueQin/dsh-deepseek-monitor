/**
 * tsdown build for dsh-deepseek-monitor: the host-half lib (lib/index.js,
 * ESM node) plus the two browser client bundles (lib/client.js and
 ...
 *   the package-name id `dsh-deepseek-monitor` (the client-modules compose
 *   keys on the package name; keep in sync with package.json `name`),
 * - `lib/client-registry.js` serves the plugin-registry channel
 *   (dsh.plugin.json), registering with the manifest id
 *   `dsh-external/dsh-deepseek-monitor` (bundle id === plugin id).
 *
 * Both bundles replicate the official DSH client-bundle preset
 * (packages/client/tsdown.client.ts), compiled from the same
 * src/client/index.tsx:
 * - externals resolve through the loader module table at runtime
 *   (PLATFORM_MODULES seed list),
  ...
  return {
    ...
    resolveId(source: string, importer: string | undefined) {
      ...
      if (source.startsWith('.') || source.startsWith('/') || /^[A-Za-z]:[\\/]/.test(source)) {
        ...
        abs = source // bare specifiers unsupported here; inline via relative paths
      }
...
export default [
  ...
  clientBundle('dsh-deepseek-monitor', 'client.js'),
  // Plugin-registry channel: bundle id = manifest id (dsh.plugin.json `id`).
  clientBundle('dsh-external/dsh-deepseek-monitor', 'client-registry.js'),
